...
static void set_leds(int val, int blink, int step)
{
    if (step == 0 || step == 3) {
    	write_int(RED_LED_FILE, val);
    	write_int(RED_BLINK_FILE, blink);
    }

    if (step == 1 || step == 3) {
    	write_int(GREEN_LED_FILE, val);
    	write_int(GREEN_BLINK_FILE, blink);
    }

    if (step == 2 || step == 3) {
    	write_int(BLUE_LED_FILE, val);
